# Roadmap

| Gate | State | Exit condition |
|---|---|---|
| Documentation foundation | complete | Scope, security and publication boundaries are public |
| Zig server alignment | blocked | Canonical protocol version and ownership are approved |
| Public extraction design | waiting | File-level allowlist and license audit are accepted |
| Minimal Godot/OpenXR shell | waiting | Original isolated shell runs without proprietary code |
| Local connectivity fixture | waiting | Synthetic loopback flow respects server authority |
| Headset validation | waiting | Named device and runtime evidence are recorded |
| Template release | waiting | Fresh clone, license, secrets and documentation gates pass |

No waiting gate implies implementation or compatibility.
